...
class Wordpress {
	...
	public function __construct() {
		...
		if ( isset( $this->options['login_sms'] ) ) {
			...
			add_action( 'init', array( $this, 'register_wp_session' ) );
		}
		...
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			...
			if ( $_GET['step'] == 1 ) {
				...
				if ( Option::getOption( 'mobile_verify', true ) and ! get_user_meta( $user->ID, 'mobile_verified', true ) ) {
					...
					$result['error'] = 'no';
					$result['text']  = __( 'Security code have beed send in your mobile. please enter the code for login to wordpress.', 'wp-sms-pro' );
					$this->json_exit( $result );
				}
