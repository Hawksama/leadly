...
class Otp {
	...
	public function __construct() {
		...
		if ( Option::getOption( 'wc_otp_enable', true ) ) {
			...
			add_action( 'wp_ajax_nopriv_wp_sms_woocommerce_otp', array( $this, 'wp_sms_woocommerce_otp' ) );
			add_action( 'wp_ajax_wp_sms_woocommerce_otp', array( $this, 'wp_sms_woocommerce_otp' ) );
			add_action( 'wp_init', array( $this, 'start_session' ) );
		}
	}
	...
	function place_order_button_content() {
		...
		if ( $this->wc_mobile_field ) {
			wp_enqueue_style( 'jquery-confrim', WP_SMS_PRO_URL . 'assets/js/jquery-confirm/jquery-confirm.min.css', false, WP_SMS_PRO_VERSION );
			wp_enqueue_script( 'jquery-confrim', WP_SMS_PRO_URL . 'assets/js/jquery-confirm/jquery-confirm.min.js', array( 'jquery-core' ), WP_SMS_PRO_VERSION );
			...
			wp_localize_script( 'wp-sms-woocommerce-otp', 'wp_sms_woocommerce_otp', array(
				...
				'lang'                => array(
					'title'       => __( '<h3>Mobile Verification</h3><h4>We send you a verification code.</h4>', 'wp-sms-pro' ),
					'submit'      => __( 'Confirm', 'wp-sms-pro' ),
					'wait'        => __( 'Loading...', 'wp-sms-pro' ),
					'number'      => __( 'Enter Your number...', 'wp-sms-pro' ),
					'code'        => __( 'OTP code...', 'wp-sms-pro' ),
					'error'       => __( 'Error', 'wp-sms-pro' ),
					'submit_code' => __( 'Verify', 'wp-sms-pro' ),
					'retry'       => __( 'Click Here', 'wp-sms-pro' ),
					'ajax_error'  => __( 'Error occured. please try again', 'wp-sms-pro' )
				),
			) );
		}
	 ...
	 * OTP Ajax Call
	 */
	function wp_sms_woocommerce_otp() {
		// Get What step we must handle and some user info
		$step             = isset( $_GET['step'] ) ? $_GET['step'] : 0;
		$this->phone      = isset( $_GET['wp_sms_otp_number'] ) ? $_GET['wp_sms_otp_number'] : null;
		...
		if ( $step == 1 ) {
			...
			$result['text'] = __( 'OTP Code sent to your number!<br>Now Enter your OTP code and Complete the Verification.', 'wp-sms-pro' );
			$this->json_exit( $result );
		} else if ( $step == 2 ) {
			$otp_code = isset( $_GET['wp_sms_otp_code'] ) ? trim( $_GET['wp_sms_otp_code'] ) : '';
			if ( ! $otp_code or empty( $otp_code ) ) {
				$result['error'] = 'yes';
				$result['text']  = __( 'OTP code is empty!', 'wp-sms-pro' );
				$this->json_exit( $result );
			}
			if ( isset( $this->options[ $this->meta_phone ] ) ) {
				if ( $this->options[ $this->meta_phone ]['code'] == $otp_code ) {
					...
					$result['text'] = __( 'OTP Verification completed.<br>Now You can place your order!', 'wp-sms-pro' );
					$this->json_exit( $result );
				} else {
					$result['error'] = 'yes';
					$result['text']  = __( 'OTP code is not valid, Please try again!', 'wp-sms-pro' );
					$this->json_exit( $result );
				}
			...
			$result['text'] = __( 'SMS Resent! Please Check Your Phone.', 'wp-sms-pro' );
			$this->json_exit( $result );
		}
	...
	function phone_validation( $data, $errors ) {
		...
		$post_user_mobile  = isset( $_POST[ $this->wc_mobile_field ] ) ? $_POST[ $this->wc_mobile_field ] : '';
