...
class smsto extends \WP_SMS\Gateway
{
    ...
    public function GetCredit()
    {
        ...
        $response = wp_remote_get($this->tariff . 'api/balance?has_key=' . $this->has_key);
        if (!is_wp_error($response) && 200 == wp_remote_retrieve_response_code($response)) {
            $body = json_decode(wp_remote_retrieve_body($response));

            return round($body->balance, 2) . ' credits';
        } else {
            return new \WP_Error('account-credit', 'Unable to send your message(s)');
        }
    }
