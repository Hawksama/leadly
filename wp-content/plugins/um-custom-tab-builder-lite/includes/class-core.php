...
class UMCTB_Core {
	...
	public function setup_metabox() {
		...
		$cmb = new_cmb2_box( array(
			'id'           => $prefix . 'metabox',
			'title'        => __( 'Tab Details', 'um-custom-tab-builder-lite-lite' ),
		...
		$cmb->add_field( array(
			'name' => __( 'Icon', 'um-custom-tab-builder-lite-lite' ),
			'id' => $prefix . 'icon',
			'type'    => 'icon_picker',
			'options' => array(
		...
		$cmb->add_field( array(
			'name'        => __( 'Slug', 'um-custom-tab-builder-lite-lite' ),
			'id'          => $prefix . 'slug',
			'type'        => 'text_medium',
			'description' => __( 'Choose the slug for the tab', 'um-custom-tab-builder-lite-lite' )
		) );

		$cmb->add_field( array(
			'name'               => __( ' Private Tab', 'um-custom-tab-builder-lite-lite' ),
			'id'                 => $prefix . 'private',
			'type'               => 'checkbox',
			'desc'               => __( 'Only owners of the profile can view tab.', 'um-custom-tab-builder-lite-lite' ),
		) );

		$cmb->add_field( array(
			'name'              => __( 'Roles with this tab', 'um-custom-tab-builder-lite-lite' ),
			'id'                => $prefix . 'roles_own',
			'type'              => 'multicheck',
			'select_all_button' => true,
			'options'           => $this->get_user_roles(),
			'description'       => __( 'Leave unchecked to allow all.', 'um-custom-tab-builder-lite-lite' )
		) );

		$cmb->add_field( array(
			'name'               => __( ' Roles that can view this tab', 'um-custom-tab-builder-lite-lite' ),
		...
		$type_options = array();
		$type_options['profile'] = __( 'Profile', 'um-custom-tab-builder-lite-lite' );
		if ( class_exists( 'UM_Groups' ) ) {
			//$type_options['profile'] = __( 'Group', 'um-custom-tab-builder-lite-lite' );
		}
		$cmb->add_field( array(
			'name'    => __( 'Tab Type', 'um-custom-tab-builder-lite-lite' ),
			'id'      => $prefix . 'tab_type',
			'type'    => 'select',
			'default' => 'profile',
			'options' => $type_options,
			'description' => __( 'What type of tab is being created? Pro version and Groups extension needed to use group tab.', 'um-custom-tab-builder-lite-lite' )
		) );

		$cmb->add_field( array(
			'name'    => __( 'Content Type', 'um-custom-tab-builder-lite-lite' ),
			'id'      => $prefix . 'content_type',
			'type'    => 'select',
			'options' => $this->get_content_types(),
			'description' => __( 'What type of content will be placed in tab? UM Form not available for group tab types.', 'um-custom-tab-builder-lite-lite' )
		) );


		$cmb->add_field( array(
			'name'    => __( 'Shortcode', 'um-custom-tab-builder-lite-lite' ),
			'id'      => $prefix . 'type_shortcode',
			'type'    => 'textarea',
			'classes' => 'um-ctb--hide',
			'description' => __( 'Add shortcodes and shorttags here. Use UM Form type for UM Forms.', 'um-custom-tab-builder-lite-lite' )
		) );

		$cmb->add_field( array(
			'name'    => __( 'Content', 'um-custom-tab-builder-lite-lite' ),
			'id'      => $prefix . 'type_content',
			'type'    => 'wysiwyg',
			'classes' => 'um-ctb--hide',
			'description' => __( 'Add html content, shortcodes and shorttags here.', 'um-custom-tab-builder-lite-lite' )
		) );
	}
	...
	public function get_content_types() {
		$content_types = array(
			'shortcode' => __( 'Shortcode', 'um-custom-tab-builder-lite-lite' ),
			'content'   => __( 'Content', 'um-custom-tab-builder-lite-lite' ), 
		);
